...
public class patient_chartPanel extends JPanel implements chartMonitor{
	private cholestrolModel model;
    private JFreeChart chart;

    //To plot the charts
    public patient_chartPanel(cholestrolModel model, String userName) throws JSONException {
    	this.model = model;
  ...
  //Function to update Dataset
    public XYDataset updateDataset(String userName, Double threshold) throws JSONException {
    	TimeSeriesCollection dataset = new TimeSeriesCollection();
        List[] data = model.getPatientCholestrol(userName);
        if (data.length == 0) {
        	return dataset;
        ...
        if (values.get(values.size()-1) > threshold) {
        	systemAlert("This guys is gonna die " + values.get(values.size()-1));
        }
        dataset.addSeries(s1);
        return dataset;
    }
	...
	public XYDataset updateData(String userName, Double threshold) throws JSONException {
		this.model = model;
    	XYDataset data = updateDataset(userName, threshold);
    	JFreeChart chart = ChartFactory.createTimeSeriesChart("Cholestrol Level", "yyyy-mm-dd hh:mm:ss", "Cholestrol Level", data, true, true, false);
        XYPlot plot = chart.getXYPlot();
        ValueAxis domain = plot.getDomainAxis();
        domain.setAutoRange(true);
	...
	@Override
	//Throws patient system alert
	public void systemAlert(String alertMsg) {
		patient_systemAlert alert = new patient_systemAlert();
